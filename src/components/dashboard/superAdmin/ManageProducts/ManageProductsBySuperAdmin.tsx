...
const ManageProductBySuperAdmin = ({
  data,
  meta,
}: {
  ...
  const handleUpdate = (id: string) => {
    router.push(`/superAdmin/products/update-product/${id}`);
  };
  ...
  const columns: ColumnDef<TProductResponse>[] = [
    ...
    },
    {
      accessorKey: "price",
      header: () => (
        <div className="cursor-pointer" onClick={() => handleSort("price")}>
    ...
    },
    {
      ...
      header: () => (
        <div className="cursor-pointer" onClick={() => handleSort("quantity")}>
          Quantity{" "}
          {sorting.key === "quantity" ? (sorting.desc ? "↓" : "↑") : ""}
        </div>
      ),
...
export default ManageProductBySuperAdmin;
